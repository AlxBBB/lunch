...
public class RestaurantServiceTest extends AbstractServiceTest {
    ...
    public void testSaveMenu() throws Exception {
        Menu newMenu = new Menu(null, LocalDate.now().plusDays(3), RESTAURANT1);
        newMenu.setDishes(Arrays.asList(new Dish("Блюдо", 1000), new Dish("Напиток", 300)));
        newMenu = service.saveMenu(newMenu);
        MATCHER_MENU.assertEquals(newMenu, service.getMenu(RESTAURANT1_ID, LocalDate.now().plusDays(3)));
    }
